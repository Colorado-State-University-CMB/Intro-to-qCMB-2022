...
ToothGrowth_df2 <- ToothGrowth %>%
  group_by(supp, dose) %>%
  summarize(n=n(), mean = mean(len), sd = sd(len)) %>%
  dplyr::rename(supplementary_type = supp) %>%
  mutate("dose (ug/day)" = dose * 1000) %>%
  select(-dose) %>% 
  arrange(mean)
...
DGE_df1 <- data %>%
    gather(key="condition_replicate", value = "counts", 2:5) %>%
    spread(key = condition_replicate, value = counts)
...
**11. Create a new column called regulation that is equal to regulation(log2FC).** This is a custom function that returns "upregulated", "downregulated" or "unchanged" depending on whether the gene has increased, decreased, or unchanged gene expression between the exponential and stationary growth phase samples. 
...
regulation <- function(log2FC, pvalue){
  if(log2FC > 1 & pvalue < 0.05){return('upregulated')}
  else if(log2FC < -1 & pvalue < 0.05){return('downregulated')}
...
DGE_df2 <- data %>%
  merge(y=gene_lengths, by = 'gene_id', all.x=TRUE) %>%
  ...
  rowwise() %>%
    ...
    mutate(pval = get_pvalue(expo_RPKM_rep1, expo_RPKM_rep2, stat_RPKM_rep1, stat_RPKM_rep2)) %>%
...
**4. Change color = supp to fill = supp in the aes() function and scale_color_manual(...) to scale_fill_manual(...). Run the code and look at the plot.*** 
...
DGE_df3 <- data %>%
 ...
 rowwise() %>%
  ...
  mutate(pvalue = get_pvalue(expo_RPKM_rep1,
                             ...
                             stat_RPKM_rep2) )%>%
  select(-expo_RPKM_rep1, -expo_RPKM_rep2,-stat_RPKM_rep1,-stat_RPKM_rep2) %>%
  mutate(regulation = regulation(log2FC = log2FC, pvalue = pvalue)) %>%
